...
macro tag2_str(str) Tag{2}(str) end
macro tag3_str(str) Tag{3}(str) end
macro tag4_str(str) Tag{4}(str) end
...
function find_language_tag(tag::Tag{4})
  tag = uppercase(tag)
  haskey(language_tag_names_opentype, tag) ? tag : nothing
end

function find_language_tag(tag::Tag{3})
  tag = lowercase(tag)
  ...
  find_language_tag(Tag{4}((tag.data..., ' ')))
end

function find_language_tag(tag::Tag{2})
  tag = lowercase(tag)
...
function find_script_tag(tag::Tag{4})
